...
export default function UnloadingDetails({ unloading, onClose }: Props) {
  ...
  // Generate PDF for the unloading
  const handlePdfCreate = async () => {
    ...
    try {
      const blob = await pdf(
        <UnloadingPDFReport
          key={Date.now()}
          vehicle={unloading.vehicle ?? { id: "", name: "N/A" }}
          packages={unloading.shipments ?? []}
          missingPackages={unloading.missingTrackings ?? []}
          unScannedPackages={unloading.unScannedTrackings ?? []}
          subsidiaryName={unloading.subsidiary?.name ?? "N/A"}
          unloadingTrackigNumber={unloading.trackingNumber ?? "N/A"}
        />
      ).toBlob();
      ...
      const fileName = `PMY_Desembarque_${unloading.subsidiary?.name ?? "Unknown"}_${currentDate}.pdf`;
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span>Detalles de Desembarque</span>
          </div>
          <div className="flex items-center gap-1 text-sm text-gray-500">
            <MapPin className="h-5 w-5" />
            <span>Sucursal: {unloading.subsidiary?.name ?? "N/A"}</span>
          </div>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <strong>Unidad de Transporte:</strong> {unloading.vehicle?.name ?? "N/A"}
            </div>
            <div>
              <strong>Fecha:</strong> {unloading.date ? new Date(unloading.date).toLocaleDateString("es-ES") : "N/A"}
            </div>
            <div>
              <strong>No. Seguimiento:</strong> {unloading.trackingNumber ?? "N/A"}
            </div>
          </div>
        </div>
        ...
        {/* Missing and Unscanned Trackings */}
        {(unloading.missingTrackings?.length > 0 || unloading.unScannedTrackings?.length > 0) && (
          <div className="mt-6 space-y-2">
            <h3 className="text-lg font-semibold text-gray-800">Guías No Validadas</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {unloading.missingTrackings?.length > 0 && (
                <div>
                  ...
                  <ul className="border border-gray-300 rounded-md divide-y divide-gray-300">
                    {unloading.missingTrackings.map((tracking, i) => (
              ...
              {unloading.unScannedTrackings?.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-gray-700">Guías Sin Escaneo</h4>
                  <ul className="border border-gray-300 rounded-md divide-y divide-gray-300">
                    {unloading.unScannedTrackings.map((tracking, i) => (
