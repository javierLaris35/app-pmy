...
export const generateWarehouseExcel = async (
  ...
  opts?: { sheetName?: string; title?: string; fileNamePrefix?: string }
): Promise<Buffer | Blob | void> => {
  const sheetName = opts?.sheetName ?? "Recepción de Envíos";
  const fileNamePrefix = opts?.fileNamePrefix ?? "Recepcion_Bodega";
  ...
  if (shouldDownload) {
    ...
    anchor.href = url;
    anchor.download = `${fileNamePrefix}_${session.id.slice(0, 8)}.xlsx`;
