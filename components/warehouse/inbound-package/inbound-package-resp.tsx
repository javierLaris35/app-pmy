...
export default function InboundPackage() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Verificamos si el usuario ya está escribiendo en algún campo de texto
      ...
      if (e.key === "F1") {
        ...
        e.preventDefault();
        // Ahora usamos tu nueva validación estricta para permitir el F2
        if (isReadyToFinish) {
          toggleModal("signatures", true);
        } else {
          // Opcional: Avisar por qué no hace nada
          safeSpeak("Faltan datos para finalizar"); 
        }
      } 
      else if (e.key === "F3") {
        e.preventDefault();
        // Como DataTable genera su propio input, lo buscamos por su placeholder
        ...
        // Auto-enfoca el escáner si el usuario empieza a teclear en la nada
        inputRef.current?.focus();
      }
    };
    ...
    // Limpieza del evento al desmontar o actualizar dependencias
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [isReadyToFinish, safeSpeak]);
  ...
  const handleCompleteSession = async () => {
    ...
    try {
      ...
      console.log("🚀 ~ handleCompleteSession ~ payload:", payload)
      ...
      // 3. Limpiamos la sesión tras el éxito
      setSession({
        id: crypto.randomUUID(),
        vehicle: null,
        startTime: new Date(),
      ...
      // Aquí puedes agregar un toast de error si tienes uno
    } finally {
      setIsScanning(false);
      setTimeout(() => inputRef.current?.focus(), 100);
    }
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-xs text-muted-foreground">
            ...
            <span className="font-mono">CP: <b>{pkg.recipientZip}</b></span>
          </div>
        )
      },
      filterFn: (row, id, value: string[]) => {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex gap-1.5 flex-wrap">
            ...
            {pkg.hasPayment && <Badge className="font-bold text-[10px] px-1.5 py-0 bg-amber-100 text-amber-800 border-amber-200 border hover:bg-amber-200" variant="secondary">
    ...
    <div className="text-slate-900 min-h-screen pt-4"> 
        ...
        <div className="flex flex-col gap-6 mb-4">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            {/* Derecha: Acciones (Atajos + Selector) */}
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <div className="w-full md:w-[250px]">
                <SucursalSelector
                  value={selectedWarehouse || user?.subsidiary?.id || user?.subsidiaryId || ""}
                  returnObject={true}
                  onlyWarehouses={true}
                  onValueChange={(val) => {
            ...
            <Card className="border-none shadow-none bg-transparent">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  <h2 className="text-lg font-semibold">Paquetes Escaneados</h2>
                </div>
              </div>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => setScanInput(e.target.value)}
                      onKeyDown={handleKeyPress}
                      className="h-12 text-base pl-10 pr-10 font-mono"
                      disabled={isScanning}
                    />
                    {scanInput && (
                      <button
              ...
              {packages.map((pkg: any) => (
                <TableRow key={pkg.id}>
                  ...
                  <TableCell className="text-right text-[10px]">
                    {pkg.hasPayment ? `$${Number(pkg.paymentAmount).toLocaleString("es-MX", { minimumFractionDigits: 2 })}` : 'OK'}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
